...
const BookingCard = ({ booking, isMentor, onAction, isProcessing }) => {
  ...
  const canPerformAction = (booking, action) => {
    if (isMentor) {
      if (booking.mentor?.id !== user?.profile_id) return false;
      switch (action) {
        case 'accept':
        case 'decline':
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-white text-lg">
                {isMentor ? `${booking.mentee_name || 'Mentee'}` : `${booking.mentor_name || 'Mentor'}`}
              </h3>
  ...
  const renderLoading = () => (
    <div className="flex items-center justify-center min-h-[50vh]">
      <div className="text-center">
        ...
        <p className="text-secondary text-lg">Loading Your Sessions...</p>
      </div>
    </div>
  );
